...
export default function DampakPage() {
  ...
  const stats = [
    ...
    { icon: Clock, lbl: "Respons Tercepat", val: 24, suffix: " jam" },
    { icon: PieChart, lbl: "Rata-rata Skor Prioritas", val: avgSkor, suffix: "" },
  ];

  return (
    <main className="mx-auto max-w-[1160px] px-3.5 sm:px-6 py-6 sm:py-12">
      <Reveal className="mb-6 sm:mb-10 text-center">
        <p className="text-xs font-bold uppercase tracking-[.12em] text-brand-600">Dampak Publik</p>
        <h1 className="mt-2 font-display text-2xl sm:text-3xl md:text-4xl font-extrabold">Dampak Penanganan SIGAP</h1>
        <p className="mx-auto mt-2 sm:mt-3 max-w-[560px] text-xs sm:text-sm text-ink-500">Data sebaran laporan, kategori masalah, dan tingkat penyelesaian.</p>
      </Reveal>
          ...
          return (
            <Reveal key={s.lbl} delay={i * 70} className="card-hover rounded-2xl bg-surface p-5 shadow-[var(--shadow-card)]">
              ...
              <p className="font-display text-3xl font-extrabold text-brand-600">
                <Counter to={s.val} suffix={s.suffix} decimals={s.lbl.includes("Prioritas") ? 1 : 0} />
              </p>
